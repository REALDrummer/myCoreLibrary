...
public class myList<T extends Comparable<? super T>> implements Comparable<myList<T>>, Cloneable, Iterable<T> {
    private T data = null;
    private myList<T> left = null, right = null, root = null;

    // DONE: improved containsAND and containsOR efficiency by avoiding toArray() calls and parsing through given list structures instead
    ...
    private int add(T object, int current_index) {
        ...
        if (data.compareTo(object) <= 0)
            ...
            if (!hasRight() || !right.isFull() || hasLeft() && left.isFull() && right.length() <= left.length())
                ...
                if (lowest_right.data.compareTo(object) < 0) {
            ...
            if (!hasLeft() || !left.isFull() || hasRight() && right.isFull() && left.length() <= right.length())
                ...
                if (highest_left.data.compareTo(object) > 0) {
